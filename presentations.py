...
detector = HandDetector(detectionCon=0.8,maxHands=1)# Change the detectionCon value to adjust the hand detection accuracy
...
while True:
    ...
    cv2.line(img,(0,gestureThreshold),(width,gestureThreshold),(0,0,255),2) # Draw a line at the gesture threshold
    
    if hands and buttonPress is False: # If hands are detected and button is not pressed
        hand = hands[0]
        ...
        if cy < gestureThreshold: # If the hand is above the threshold
            ...
            # GESTURE -1 :If the thumb finger is up
            ...
            # GESTURE -2 :If the pinky finger is up
    ...
    current_image[0:heightSmall, width-widthSmall:width] = imageSmall # Add the webcam image to the current image
